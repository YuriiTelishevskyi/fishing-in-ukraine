...
| `GA_MEASUREMENT_ID` | `G-XXXXXXX` | Google Analytics 4 (безкоштовний). Залиште порожнім, щоб вимкнути аналітику. Consent-gated: gtag завантажується лише після того, як користувач натисне «Прийняти» в банері cookie. |
...
- **Кешування `/uploads`** — завантажені файли мають контент-адресовані (uuid) імена, тому віддаються з `Cache-Control: max-age=31536000, immutable` (1 рік) для агресивного браузерного кешування.
